...
function generateRandomTransactions(storeName) {
  ...
  // Згенеруємо 3–5 випадкових транзакцій
  const count = Math.floor(Math.random() * 3) + 3;

  for (let i = 0; i < count; i++) {
    const amount = Math.floor(Math.random() * 9901) + 100; // 100–10000
    const cashback = +(amount * 0.02).toFixed(2); // 2%
    totalCashback += cashback;
    monthCashback += cashback;

    const li = document.createElement('li');
    li.innerHTML = `<span>${storeName}</span> — ₴${amount.toFixed(2)} <strong>+₴${cashback} кешбек</strong>`;
    transactionList.prepend(li); // додаємо зверху
  }
